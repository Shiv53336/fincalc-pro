...
class PdfService {
  ...
  /// One-shot: generate, save, and share — with free tier limit check.
  static Future<void> generateAndShare(
    ...
    // Check PDF export limit for free users
    if (!PremiumManager.isPremium) {
      final limitInfo = await NudgeService.checkPdfLimit();
      final canExport = limitInfo['canExport'] == 1;
      final remaining = limitInfo['remaining'] ?? 0;
      ...
      if (!canExport) {
        if (context.mounted) {
          await PremiumNudgeSheet.showIfEligible(
            context,
            nudgeId: 'pdf_limit',
            headline: 'Monthly PDF Limit Reached',
            body: 'You\'ve used all 3 free exports this month.\nUpgrade to Premium for unlimited PDF exports.',
            ctaLabel: 'Unlock Unlimited PDFs — Rs.99',
          );
        }
        return;
      }

      // Warn when on last free export
      if (remaining == 1 && context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Last free PDF export this month. Upgrade for unlimited.'),
      ...
      await NudgeService.incrementPdfCount();
    }
